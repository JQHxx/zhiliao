...
import com.dev.rexhuang.zhiliao_core.player2.model.MusicProviderSource;
            ...
            new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intent.getAction())) {
                        Logger.t(TAG).d( "Headphones disconnected.");
                        if (isPlaying()) {
                            Intent i = new Intent(context, MusicCompatService.class);
                            i.setAction(MusicCompatService.ACTION_CMD);
                            i.putExtra(MusicCompatService.CMD_NAME, MusicCompatService.CMD_PAUSE);
...
//                        new DefaultLoadControl());
    ...
    public void seekTo(long position) {
        Logger.t(TAG).d( "seekTo called with ", position);
        if (mExoPlayer != null) {
            registerAudioNoisyReceiver();
    ...
    private void tryToGetAudioFocus() {
        Logger.t(TAG).d( "tryToGetAudioFocus");
        int result =
    ...
    private void giveUpAudioFocus() {
        Logger.t(TAG).d( "giveUpAudioFocus");
    ...
    private void configurePlayerState() {
        Logger.t(TAG).d( "configurePlayerState. mCurrentAudioFocusState=", mCurrentAudioFocusState);
    ...
    private final AudioManager.OnAudioFocusChangeListener mOnAudioFocusChangeListener =
            new AudioManager.OnAudioFocusChangeListener() {
                ...
                public void onAudioFocusChange(int focusChange) {
                    Logger.t(TAG).d( "onAudioFocusChange. focusChange=", focusChange);
                    switch (focusChange) {
    ...
    private void releaseResources(boolean releasePlayer) {
        Logger.t(TAG).d( "releaseResources. releasePlayer=", releasePlayer);
    ...
    private final class ExoPlayerEventListener implements Player.EventListener {
        ...
        public void onPlayerError(ExoPlaybackException error) {
            ...
            Logger.t(TAG).e( "ExoPlayer error: what=" + what);
            if (mCallback != null) {
