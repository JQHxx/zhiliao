...
public class MusicManager {
    ...
    /**
     * 根据musicId播放,调用前请确保已经设置了播放列表,会先判断播放列表有没有这首歌,若无则不播放
     */
    public void playMusicById(String musicId) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            if (MusicProvider.getInstance().hasMusicEntity(musicId)) {
                connection.getTransportControls().playFromMediaId(musicId, null);
            }
        }
    }

    /**
     * 根据 MusicEntity 播放，实际也是根据 musicId 播放
     */
    public void playMusicByEntity(MusicEntity musicEntity) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            MusicProvider.getInstance().addMusicEntity(musicEntity);
            connection.getTransportControls().playFromMediaId(musicEntity.getId(), null);
    ...
    public void playMusicByIndex(int index) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            List<MusicEntity> list = MusicProvider.getInstance().getMusicEntityList();
            if (list != null && index >= 0 && index < list.size()) {
                connection.getTransportControls().playFromMediaId(list.get(index).getId(), null);
            }
        }
    }
     ...
     * 播放
     *
     * @param musicEntities 播放列表
     * @param index         要播放的歌曲在播放列表中的下标
     */
    public void playMusic(List<MusicEntity> musicEntities, int index) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        Logger.d(connection.isConnected());
        if (connection.isConnected()) {
            MusicProvider.getInstance().setMusicList(musicEntities);
            connection.getTransportControls().playFromMediaId(musicEntities.get(index).getId(), null);
        }
    }
     ...
     * 准备播放，根据musicId
     */
    public void prepareFromSongId(String musicId) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            connection.getTransportControls().prepareFromMediaId(musicId, null);
        }
    }
    ...
    public List<MusicEntity> getPlayList() {
        return MusicProvider.getInstance().getMusicEntityList();
    }

    /**
     * 更新播放列表
     */
    public void updatePlayList(List<MusicEntity> musicEntities) {
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            MusicProvider.getInstance().setMusicList(musicEntities);
        }
     ...
     * 获取当前播放的歌曲信息
     */
    public MusicEntity getNowPlayingSongInfo() {
        MusicEntity musicEntity = null;
        MediaSessionConnection connection = MediaSessionConnection.getInstance();
        if (connection.isConnected()) {
            MediaMetadataCompat metadataCompat = connection.getNowPlaying();
            if (metadataCompat != null) {
                String musicId = metadataCompat.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID);
                musicEntity = MusicProvider.getInstance().getMusicEntity(musicId);
                //播放列表改变了或者清空了，如果还在播放歌曲，这时候 getMusicEntity 就会获取不到，
                //此时需要从 metadataCompat 中获取
                if (musicEntity == null && !TextUtils.isEmpty(musicId)) {
                    musicEntity = getSongInfoFromMediaMetadata(metadataCompat);
                }
            }
        }
        return musicEntity;
    ...
    private MusicEntity getSongInfoFromMediaMetadata(MediaMetadataCompat metadata) {
        MusicEntity musicEntity = new MusicEntity();
        musicEntity.setId(metadata.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID));
        musicEntity.setNormal(metadata.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_URI));
//        musicEntity.setAlbumName(metadata.getString(MediaMetadataCompat.METADATA_KEY_ALBUM));
        SingersEntity singersEntity = new SingersEntity();
        singersEntity.setName(metadata.getString(MediaMetadataCompat.METADATA_KEY_ARTIST));
        musicEntity.setSingers(Arrays.asList(singersEntity));
//        musicEntity.setDuration(metadata.getLong(MediaMetadataCompat.METADATA_KEY_DURATION));
//        musicEntity.setGenre(metadata.getString(MediaMetadataCompat.METADATA_KEY_GENRE));
        musicEntity.setCover(metadata.getString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI));
//        musicEntity.setAlbumCover(metadata.getString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI));
        musicEntity.setName(metadata.getString(MediaMetadataCompat.METADATA_KEY_TITLE));
//        musicEntity.setTrackNumber((int) metadata.getLong(MediaMetadataCompat.METADATA_KEY_TRACK_NUMBER));
//        musicEntity.setAlbumSongCount((int) metadata.getLong(MediaMetadataCompat.METADATA_KEY_NUM_TRACKS));
//        musicEntity.setSongCoverBitmap(metadata.getBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART));
        return musicEntity;
    }
    ...
    public String getNowPlayingSongId() {
        ...
        if (connection.isConnected()) {
            MediaMetadataCompat metadataCompat = connection.getNowPlaying();
            if (metadataCompat != null) {
                musicId = metadataCompat.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID);
            }
        }
        return musicId;
    }
    ...
    public int getNowPlayingIndex() {
        int index = -1;
        String musicId = getNowPlayingSongId();
        if (!TextUtils.isEmpty(musicId)) {
            index = MusicProvider.getInstance().getIndexByMusicEntity(musicId);
        }
        return index;
    }
     ...
     * 判断传入的音乐是不是正在播放的音乐
     */
    public boolean isCurrMusicIsPlayingMusic(String musicId) {
        if (TextUtils.isEmpty(musicId)) {
            return false;
        } else {
            MusicEntity playingMusic = getNowPlayingSongInfo();
            return playingMusic != null && musicId.equals(playingMusic.getId());
        }
    }
     ...
     * 判断传入的音乐是否正在播放
     */
    public boolean isCurrMusicIsPlaying(String musicId) {
        return isCurrMusicIsPlayingMusic(musicId) && isPlaying();
    }

    /**
     * 判断传入的音乐是否正在暂停
     */
    public boolean isCurrMusicIsPaused(String musicId) {
        return isCurrMusicIsPlayingMusic(musicId) && isPaused();
    }
     ...
     * 扫描本地媒体信息
     */
    public List<MusicEntity> querySongInfoInLocal() {
        ...
        while (cursor.moveToNext()) {
            MusicEntity musicEntity = new MusicEntity();
//            musicEntity.setAlbumId(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ALBUM_ID)));
//            musicEntity.setAlbumCover(getAlbumArtPicPath(sContext, musicEntity.getAlbumId()));
//            musicEntity.setSongNameKey(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.TITLE_KEY)));
//            musicEntity.setArtistKey(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ARTIST_KEY)));
//            musicEntity.setAlbumNameKey(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ALBUM_KEY)));
            SingersEntity singersEntity = new SingersEntity();
            singersEntity.setName(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ARTIST)));
            musicEntity.setSingers(Arrays.asList(singersEntity));
//            musicEntity.setArtist(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ARTIST)));
//            musicEntity.setAlbumName(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.ALBUM)));
            musicEntity.setNormal(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DATA)));
//            musicEntity.setDescription(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DISPLAY_NAME)));
            musicEntity.setName(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.TITLE)));
//            musicEntity.setMimeType(cursor.getString(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.MIME_TYPE)));
//            musicEntity.setYear(String.valueOf(cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.YEAR))));
//            musicEntity.setDuration(cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DURATION)));
//            musicEntity.setSize(String.valueOf(cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.SIZE))));
//            musicEntity.setPublishTime(String.valueOf(cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DATE_ADDED))));
//            musicEntity.setModifiedTime(String.valueOf(cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DATE_MODIFIED))));
            String musicId = !TextUtils.isEmpty(musicEntity.getNormal()) ? MD5.hexdigest(musicEntity.getNormal())
